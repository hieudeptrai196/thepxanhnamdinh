...
export function NewsCard({ article, featured = false }: Props) {
  ...
  if (featured) {
    return (
      <Link href={`/news/${article.slug}`} className="block col-span-full">
        <article className="group bg-bg-secondary rounded-[var(--radius-default)] border border-[var(--border-color)] overflow-hidden transition-shadow duration-150 hover:shadow-card">
          <div className="grid sm:grid-cols-2 gap-0">
            ...
            <div className="p-5 sm:p-6 lg:p-8 flex flex-col justify-center">
              ...
              <h2 className="font-heading font-bold text-text-primary text-lg sm:text-xl lg:text-2xl leading-tight mb-3 line-clamp-3 group-hover:text-club-blue transition-colors duration-150">
                {article.title}
              </h2>
  ...
  return (
    <Link href={`/news/${article.slug}`} className="block">
      <article className="group bg-bg-secondary rounded-[var(--radius-default)] border border-[var(--border-color)] overflow-hidden transition-shadow duration-150 hover:shadow-card h-full">
        ...
        <div className="p-4 sm:p-5">
          ...
          <h3 className="font-heading font-bold text-text-primary text-sm sm:text-base leading-tight mb-2 line-clamp-2 group-hover:text-club-blue transition-colors duration-150">
            {article.title}
          </h3>
